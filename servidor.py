...
memoria = {
    "ultimo_valor": 0.0, "sugerencia": "⏳ IA SINCRONIZANDO", "confianza": "0%",
    "radar_rosa": "0%", "tp_s": "--", "tp_e": "--", "fase": "ANALIZANDO", "historial_visual": []
}
...
async def recibir_resultado(res: Resultado):
    v = res.valor
    if v == memoria["ultimo_valor"]: return {"status": "dup"}
    ...
    try:
        with open(FILE_DB, 'r') as f: total_hist = [float(l.strip()) for l in f.readlines() if l.strip()][-200:]
    except: total_hist = []
